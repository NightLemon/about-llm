...
def main() -> None:
    ...
    estimate_report = {
        "report_version": "about-llm.qlora-memory-estimate.v1",
        "mode": "estimate_only" if args.estimate_only else "training_preflight",
        "model": {"model_id": args.model_id, "revision": args.revision},
        "input": {
            "num_parameters": args.num_parameters,
            "num_layers": args.num_layers,
        ...
        "oom_degradation_order": list(oom_degradation_order()),
        "evidence_boundary": (
            "This is a formula-based memory estimate. It does not load the model, "
            "allocate CUDA memory, measure another process, or predict an exact OOM boundary."
        ),
    }
    if args.estimate_only:
        _print_report(estimate_report)
        return
    ...
    write_strict_json(
        args.output_dir / "qlora-memory-estimate.json", estimate_report
    )
    ...
    if args.data_preflight_only:
        _print_report(
            {
                "report_version": "about-llm.qlora-preflight.v1",
                "mode": "data_preflight",
                "status": "completed",
                "model": {"model_id": args.model_id, "revision": args.revision},
                ...
                "artifacts": [
                    str(args.output_dir / "qlora-memory-estimate.json"),
                    str(args.output_dir / "sft-data-audit.json"),
                    str(args.output_dir / "sft-training-readiness.json"),
                ],
                "evidence_boundary": (
                    "Memory was estimated and data contracts were checked; tokenizer, "
                    "model, CUDA, label projection, and optimization were not executed."
                ),
            }
        )
        return
        ...
        _print_report(
            {
                ...
                "evidence_boundary": (
                    "Memory, data, and assistant-label tokenization were checked; model "
                    "weights, CUDA, final Trainer collation, and optimization were not executed."
                ),
            }
        )
        return

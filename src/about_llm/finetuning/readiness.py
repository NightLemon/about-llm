"""Strict, reloadable SFT training-readiness artifacts.

The audit process may read every split.  A trainer consumes only this envelope
and the train-only JSONL, so held-out plaintext does not need to cross the
trainer's permission boundary.
"""
...
_EVIDENCE_BOUNDARY = (
    ...
    "permission, consent, comprehensive PII/secret detection, or absence of contamination."
)
...
class SFTTrainingReadinessReport:
    """Minimal decision envelope safe to pass to a train-only process."""
    ...
    near_duplicate_views: tuple[NearDuplicateView, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not views
            or any(not isinstance(view, NearDuplicateView) for view in views)
        ...
        if (
            isinstance(self.near_duplicate_candidate_count, bool)
            or not isinstance(self.near_duplicate_candidate_count, int)
            or self.near_duplicate_candidate_count < 0
        ):
            raise ValueError("near_duplicate_candidate_count must be a non-negative integer")
        if (
            isinstance(self.governance_blocking_finding_count, bool)
            or not isinstance(self.governance_blocking_finding_count, int)
            or self.governance_blocking_finding_count < 0
        ):
            raise ValueError(
                "governance_blocking_finding_count must be a non-negative integer"
            )
        expected_binding = _binding_fingerprint(
            self.training_manifest_fingerprint, self.combined_manifest_fingerprint
        )
        expected_training_manifest = _audit_manifest_fingerprint(
            self.training_ordered_dataset_fingerprint, ("train",)
        )
        expected_combined_manifest = _audit_manifest_fingerprint(
            self.combined_ordered_dataset_fingerprint,
            ("train", "validation", "test"),
        )
        if self.training_manifest_fingerprint != expected_training_manifest:
            raise ValueError(
                "training_manifest_fingerprint is inconsistent with the ordered dataset"
            )
        if self.combined_manifest_fingerprint != expected_combined_manifest:
            raise ValueError(
                "combined_manifest_fingerprint is inconsistent with the ordered dataset"
            )
    ...
    def from_reports(
        cls,
        binding_report: SFTTrainingBindingReport,
        near_duplicate_report: SFTNearDuplicateAuditReport,
        governance_report: SFTGovernanceAuditReport,
    ) -> SFTTrainingReadinessReport:
        if not isinstance(binding_report, SFTTrainingBindingReport):
            raise ValueError("binding_report must be SFTTrainingBindingReport")
        if not isinstance(near_duplicate_report, SFTNearDuplicateAuditReport):
            raise ValueError("near_duplicate_report must be SFTNearDuplicateAuditReport")
        if not isinstance(governance_report, SFTGovernanceAuditReport):
            raise ValueError("governance_report must be SFTGovernanceAuditReport")
        if (
            binding_report.split_report.ordered_dataset_fingerprint
            != near_duplicate_report.ordered_dataset_fingerprint
        ):
            raise ValueError(
                "near-duplicate report dataset differs from the bound split artifact"
            )
        if (
            binding_report.split_report.ordered_dataset_fingerprint
            != governance_report.ordered_dataset_fingerprint
        ):
            raise ValueError("governance report dataset differs from the bound split artifact")
        return cls(
            training_ordered_dataset_fingerprint=(
                binding_report.training_report.ordered_dataset_fingerprint
            ),
            training_manifest_fingerprint=(
                binding_report.training_report.manifest_fingerprint
            ),
            combined_ordered_dataset_fingerprint=(
                binding_report.split_report.ordered_dataset_fingerprint
            ),
            combined_manifest_fingerprint=binding_report.split_report.manifest_fingerprint,
            binding_fingerprint=binding_report.binding_fingerprint,
            near_duplicate_manifest_fingerprint=(
    ...
    def _identity(self) -> dict[str, object]:
        return {
            "artifact_version": SFT_TRAINING_READINESS_VERSION,
            ...
            "near_duplicate_audit_version": SFT_NEAR_DUPLICATE_AUDIT_VERSION,
            "near_duplicate_manifest_fingerprint": (
            ...
            "governance_audit_version": SFT_GOVERNANCE_AUDIT_VERSION,
...
def load_sft_training_readiness(path: Path) -> SFTTrainingReadinessReport:
    """Load a strict artifact and reject duplicate, missing, or unknown fields."""
    ...
    if payload["artifact_version"] != SFT_TRAINING_READINESS_VERSION:
        raise ValueError(f"{path}: unsupported readiness artifact_version")
    if payload["near_duplicate_audit_version"] != SFT_NEAR_DUPLICATE_AUDIT_VERSION:
        raise ValueError(f"{path}: unsupported near_duplicate_audit_version")
    if payload["governance_audit_version"] != SFT_GOVERNANCE_AUDIT_VERSION:
        raise ValueError(f"{path}: unsupported governance_audit_version")
    if payload["evidence_boundary"] != _EVIDENCE_BOUNDARY:
        raise ValueError(f"{path}: evidence_boundary does not match the artifact schema")
    if payload["scope"] != _SCOPE:
        raise ValueError(f"{path}: scope does not match the artifact schema")
    ...
    try:
        views = tuple(NearDuplicateView(item) for item in views_raw)
    ...
    report = SFTTrainingReadinessReport(
        ...
        training_manifest_fingerprint=_string(
            payload["training_manifest_fingerprint"], "training_manifest_fingerprint"
        ),
        ...
        combined_manifest_fingerprint=_string(
            payload["combined_manifest_fingerprint"], "combined_manifest_fingerprint"
        ),
    ...
    if gate_passed is not report.gate_passed:
        raise ValueError("gate_passed is inconsistent with blocking finding counts")
    supplied_manifest = _string(payload["manifest_fingerprint"], "manifest_fingerprint")
    _require_fingerprint(supplied_manifest, "manifest_fingerprint")
    if supplied_manifest != report.manifest_fingerprint:
        raise ValueError(
...
def validate_sft_training_readiness(
    records: tuple[SFTRecord, ...], readiness: SFTTrainingReadinessReport
) -> SFTDataAuditReport:
    """Bind a parsed train snapshot to a passing readiness envelope."""

    if not isinstance(readiness, SFTTrainingReadinessReport):
        raise ValueError("readiness must be SFTTrainingReadinessReport")
    report = validate_training_records(records)
    if report.ordered_dataset_fingerprint != readiness.training_ordered_dataset_fingerprint:
        raise ValueError("training data ordered fingerprint differs from readiness artifact")
    if report.manifest_fingerprint != readiness.training_manifest_fingerprint:
        raise ValueError("training data manifest differs from readiness artifact")
    if not readiness.gate_passed:
        raise ValueError(
            "readiness gate failed with "
...
def _binding_fingerprint(training_manifest: str, combined_manifest: str) -> str:
...
def _audit_manifest_fingerprint(

...
重试账本只保存判断所需的脱敏身份、错误类别、相对时间、终态、用量和版本；API key、认证 header、原始 Prompt、
个人信息与不透明推理状态不应默认进入普通日志。完整的脱敏、灰度、监控和回滚要求统一见
[生产检查表](../practice/production-checklist.md)，工具副作用的审批与对账见
[Agent Runtime](../applications/agent-runtime.md)。

真实小流量测试必须由操作者显式开启并限制 origin、模型版本、请求数、并发、输出、时限与费用。它要逐层记录连接、
认证、非流式、流式、取消、限流和账单对账；一次成功调用仍不能证明生产 SLO。本页不再复制完整发布清单。

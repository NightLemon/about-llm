...
如果要把这套控制接到 Gemini 应用，先把本次部署的产品平台、API 与版本、端点、型号、区域、账号计费范围和
价格快照一起冻结，再把它们写入 attempt 记录。随后可以运行[实验 0C](../practice/labs/lab-0c-cloud-budget.md)，
观察本地账本怎样预留、结算和保留未知费用。实验中的 80、66 和 146 micro-USD 是作者写定的算例：它说明应用
在证据不足时为何保留预算，不说明任何 Gemini 平台、接口或 HTTP 状态一定如何计费。
...
本地账本把第一次 500 保留为 80，是为了避免在结果未知时提前释放额度。它不是对供应商已经收费的判断；真实
attempt 仍要用目标平台和接口返回的标识、用量、取消或查询结果，并最终用对应账号的账单数据对账。

...
应用允许 Claude 使用只读工具 `get_ticket`，但模型本身没有数据库权限。先从同一张工单的纯文本基线开始：应用把
助手的行为要求放在顶层 `system`，把用户的工单查询写入 `messages` 后发送。若 Claude 直接返回文字，应用先确认
消息终态和 `usage` 都完整，再检查这段文字是否忠实回答任务。

这个基线只回答一个问题：一次 Messages 调用的协议对象是否完整。它还没有读取工单，也没有证明用户对工单有权限。
当任务确实需要查询时，才把同一请求扩展为两轮工具往返：
...
任务验收程序还要确认回答没有混入其他租户的工单，也没有声称工单已经处理完成。无论走纯文本基线还是工具往返，
都要把下面三种成功分开：

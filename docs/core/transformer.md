...
本章追踪一次具体前向计算（forward）。Tokenizer 先把输入变成 IDs；训练时要定义输入与下一个目标的错位配对，
生成时则从已有前缀继续预测。下面只跟生成路径：假设一条短 prompt 被编成 4 个 token：
...
Prefill 的最后一行 logits 用来选出 `t4`。把**选中的** `t4` 送回模型后，decode 才生成预测 `t5` 的 logits；
本例在新选中的 `t4` 命中配置的 EOS 或应用停止规则时结束，不再开始这轮 decode。后文所有专题都可以回到这两行检查，
而不必重新想象一组抽象符号。

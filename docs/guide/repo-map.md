...
同一个主题可以同时出现在正文、Lab 和项目中，但每一类只保留一种权威职责：

| 页面 | 读者来这里做什么 | 内容边界 |
|---|---|---|
| 主题正文 | 首次理解一个主问题 | 讲机制、贯穿例子、选择与边界；实现细节链接到 Lab/项目 |
| `docs/practice/labs/` | 改一个变量并观察结果 | 给最小命令、预测、输出和解释；不复制整章 |
| `docs/practice/projects/` | 判断项目在学习路径与证据层级中的位置 | 给目标、推荐顺序、首次成功和验收摘要 |
| `projects/*/README.md` | 实际运行和维护一个工程目录 | 记录安装、完整命令、文件接口、产物和故障排查 |
| `docs/reference/` | 查术语、公式、模板和治理接口 | 允许高密度检索，不伪装成必读线性课程 |
| `docs/evidence/` | 核对某项 claim 到底运行或复核了什么 | 保存版本、输入、结果、失败与不可外推边界 |

当站点项目页和项目 README 都需要一条命令时，站点页只保留“第一次成功”与学习意义，README 成为完整命令
和参数的权威入口。正文引用证据页时保留最短结论边界，证据页则把第一次阅读者引回对应正文。

...
可访问的 Anthropic、OpenAI 和 Google 推理 API。[SOURCE:arxiv-opaque-reasoning-case]
...
8 月，供应商采取缓解措施后，图 1 的结果已不能按论文第 2.4 节和附录 C 所述攻击复现。该句只转述作者在该日期对指定
结果与攻击的历史报告，不能推出所有攻击都已修复，也不能说明今天任何端点的状态。

因此，本章只把它当作历史架构案例。这里不判断当前端点是否存在同类漏洞，也不会提供针对真实供应商的提取脚本；部署前仍须按
实际供应商、协议版本、地区和数据用途审查当前文档、合同与自己的测试证据。

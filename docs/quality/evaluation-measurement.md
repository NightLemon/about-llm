...
- **适合读者**：设计人工标注、使用大模型评分（LLM-as-a-Judge）、业务评测集和模型发布实验的开发者与算法工程师。
...
测量计划要先区分**最小有意义效果**与**最小可检测效果（MDE）**：前者来自业务，后者取决于样本量、噪声、
相关结构、检验方向、显著性水平和目标功效。预算不足时，应写明实验分辨力不足，不能把 MDE 当成业务价值。

配对 sign test 中，真正提供方向信息的是去掉平局后的有差异配对；总 case 数、独立用户数和有效配对数不能互换。
精确公式、`23` 个有效配对的固定样例、聚类与序贯查看统一放在
[评测统计](../foundations/evaluation-statistics.md)。本页只要求在生成结果前把分析单位、目标效果和样本计划写进测量契约。
...
| Slice coverage | 每个硬风险切片至少收集多少 case，缺失时是阻断、补样还是只能作探索性报告 |
...
| 看到的说法 | 真正缺少的东西 |
|---|---|
| “\(\kappa\) 很高，所以参考标签正确” | 独立外部准则、干预反例和专家审查 |
...
1. 两位标注者的 \(\kappa=1\)，为什么仍不能说参考标签正确？

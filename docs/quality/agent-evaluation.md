...
不要等结果出来才决定哪些记录算进成功率。对每类 case，先写下业务目标、允许的终态和分母规则。例如，
“跨租户退款”期望的是策略在 Handler 前拒绝；它可以通过安全 verifier，却不是“退款任务成功”。相反，
用户无权限、信息不足或工具结果不确定时，升级人工可能是该 case 的正确完成状态，只要 verifier 能检查
没有发生禁止副作用、升级信息足够让人继续处理。

| case 终态 | 任务成功率 | 相应控制指标 | 不能事后怎么处理 |
|---|---|---|---|
| 业务终态已由 verifier 判定 | 进入；通过或失败均计入 | 同时记录权限、审批和 effect | 只保留通过的 case |
| 合法拒答或升级人工，且 case 预先要求它 | 由该 case 的 verifier 判定 | 记录拒答是否及时、是否无副作用 | 一律当作模型失败或成功 |
| 正常任务被策略错误拦截 | 由任务 verifier 判定，通常失败 | 计入 policy over-refusal 的分子；分母是 policy-allowed proposals | 从任务集删掉以提高安全率 |
| verifier、标签或外部 effect 无法判定 | 不进入 `task_success` 分母，并单独报告数量与原因 | `unjudged`/pending 必须让门禁失败或转人工 | 静默删行，或把未知写成通过 |

这样，拒绝率、任务成功率和安全阻止率各自仍有明确含义。特别是有意让系统少做动作的更新，必须同时看
正常任务的 over-refusal 与未判定比例；只看 harmful-compliance 降低，无法知道它是更安全还是全部拒绝。

...
- **首次阅读**：先运行“24/30”客服助手例子，再为自己的任务写 20–30 个评测样本。
- **完成信号**：能从逐样本差异、总体区间和高风险切片作出发布或回滚决定。
...
这个固定例子包含 25 条普通客服问题和 5 条“读取另一租户订单”的高风险请求。对于后五条，正确输出是
`DENY`。两套系统的总分与切片结果是：

| 系统 | 普通问题 | 跨租户拒绝 | 总分 |
|---|---:|---:|---:|
| A（Baseline） | 18/25 | 4/5 | 22/30 |
| B（Candidate） | 22/25 | 2/5 | 24/30 |

B 在四条普通问题上从错变对，却在两条跨租户请求上从 `DENY` 退化成 `ALLOW`。其余 24 条没有变化。
因此，“多答对两题”实际上是 `4 次改善 - 2 次高风险退化`。

程序对同一批评测样本做配对 bootstrap。总体正确率差是 `+0.067`，95% 区间为 `[-0.100,+0.233]`；30 条
样例还不足以确认总体改善大于零。跨租户切片从 4/5 降到 2/5，差值为 `-0.400`，区间为
`[-0.800,0.000]`。所以固定门禁会拦截 B。

这个结论不需要把所有风险硬塞进一个总分。普通问题的改善可以保留，高风险回归则要先修复并补充样例。
脚本使用仓库准备的输出，没有调用真实模型，也没有证明这 30 条样例代表线上流量。完整的证据工件和重算流程见
[Evaluation Gate](../practice/projects/evaluation-gate.md)。

这里所谓“跨租户切片”只是在五条拒绝样例上计算精确匹配，并不是一项覆盖所有攻击和权限路径的安全分数。
...
评测集不是随手收集的一袋问题。每个[评测样本](../reference/glossary.md#term-evaluation-case)都应该代表一个你愿意为之做决策的场景。
...
每条样本保存稳定 ID、输入、期望或评分规则、来源、切片和版本。稳定 ID 只是名字；如果输入或评分规则变了，
也要产生新的内容指纹。开发过程中可以反复看开发集，最终发布集则要限制查看和调参次数。

还要在看结果前规定没有得到正常评分时怎么办：超时、拒答、解析失败、工具错误和评审模型无法判断，分别记为任务失败、
单独终态，还是让本次评测无效。这个规则必须同时作用于总体和每个保护切片的分子、分母；只把“有分数”的样本
留在分母，会让一个少答或多报错的候选系统看起来更好。
...
平均分会把完全不同的失败混在一起。先为每条样本保存结果和原因，再聚合：
...
| 开放回答是否满足要求？ | 评分规则（rubric）+ 人工或校准后的评审模型 | 必须项、致命错误和证据来源 |
| 检索是否找到正确证据？ | Recall@k、MRR、nDCG | 参考证据和无答案样本的定义 |
...
仓库中的精确指标、Qwen 固定样本和引用片段反例集中在
[评测方法与发布决策](evaluation-methodology.md)和 [Evaluation Gate](../practice/projects/evaluation-gate.md)。

## 用大模型评分（LLM-as-a-Judge）

评审模型可以帮助初筛大量开放回答，但最好把它看成一位速度很快、判断会漂移的标注员。

给评审模型明确评分规则和必要证据；隐藏候选模型身份，并随机交换 A/B 顺序。然后在一批代表性样本上让专家独立评分，
比较评审模型在不同语言、长度和错误类型上的一致性。模型、Prompt、评分规则或流量变化后，这次校准需要重做。

评审模型给出的 `confidence=0.9` 或 1–5 分并不会自动成为正确概率。如果系统准备根据置信度自动放行请求，
...
截至 2026-09-20 的 LLM 复核，OpenAI 官方 [Evals guide](https://developers.openai.com/api/docs/guides/evals)[SOURCE:openai-evals]仍使用
...
在本仓库的发布工件里，总体质量和保护切片都把 bootstrap 区间下界与预先声明的门槛比较。报告中的
`probability_of_improvement` 只是当前重采样结果中差值大于零的比例，既不是 candidate 更好的概率，也不替代门禁。

样本很少时，区间往往会告诉你“目前还无法区分”。这也是有用结果：保持旧版本、补充目标 case，或选择风险更低的 canary。
多指标、多切片和反复偷看会增加误报，需要在实验前写清 family、look schedule 和停止规则。

Bootstrap、随机化检验、Holm 多重比较校正、序贯设计和发布工件的完整口径见
[评测方法与发布决策](evaluation-methodology.md)。

## 错误分析把分数变成改动

每轮从失败中抽样，建立一套足够稳定的原因分类。可以先按发生位置分为输入理解、知识与检索、推理与输出、
工具与权限，以及评分错误。

分类允许继续调整，但同一失败尽量只标一个首要原因，并记录严重度。
...
离线集稳定、便于重放；线上指标才包含真实用户、真实流量和完整系统。A/B 或灰度发布可以同时观察任务完成、
人工升级、撤销、延迟、成本和安全门槛。

实验开始前，要确定随机化单位、最大样本、运行时长、查看频率和异常中止条件。
...
3. 评审模型偏爱更长的答案时，怎样用人工样本发现并量化这种偏差？

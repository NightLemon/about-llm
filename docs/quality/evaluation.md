...
- **首次阅读**：先运行“24/30”客服助手例子，再为自己的任务写 20–30 个 case。
- **完成信号**：能从逐 case 差异、总体区间和高风险切片作出发布或回滚决定。
...
程序对同一批 case 做配对 bootstrap。总体正确率差是 `+0.067`，95% 区间为 `[-0.100,+0.233]`；30 条
...
还要在看结果前规定没有得到正常评分时怎么办：超时、拒答、解析失败、工具错误和 judge 无法判断，分别记为任务失败、
单独终态，还是让本次评测无效。这个规则必须同时作用于总体和每个保护切片的分子、分母；只把“有分数”的 case
留在分母，会让一个少答或多报错的 candidate 看起来更好。

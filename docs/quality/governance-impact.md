...
- **卡住时**：先跟着本页的招聘案例走完一遍，再用[治理工件模板](../reference/governance-templates.md)填写自己的系统。
...
假设上线前的切片检查发现：非典型职业经历的简历比基线更常被解析器漏掉技能。工程负责人把失败样本、解析器版本和
漏选率写入控制证据；招聘领域专家判断这会改变候选人的机会；用途负责人提出只保留影子运行。隐私、安全与法律审查人
分别记录数据处理、安全边界和**本地区、当前用途下**仍需确认的义务，而不是替团队宣布“已经合规”。

发布决定此时应写成可执行的限制：只允许一个岗位的影子流量，招聘人员看不到建议；运行负责人拥有关闭该路径的开关；
两周内由独立审阅人复查新增样本和申诉演练。若影子结果再次出现该切片漏选，运行负责人先停止流量，工程负责人修复
解析器，招聘领域专家复审岗位准则，随后用原失败样本和新增反例验证。只有新的证据和具名决定都到位，团队才讨论扩大范围。

这个记录链保留了人作决定的位置：模型给出建议，系统控制限制影响，具名人员根据证据批准、限制或拒绝发布。
...
[机器遗忘](../training/machine-unlearning.md)说明能够撤销到什么程度。
...
可复制的用途记录、影响评估、控制证据、发布决定、例外、事件和退役字段见[治理工件模板](../reference/governance-templates.md)。

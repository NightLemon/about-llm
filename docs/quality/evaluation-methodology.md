...
Evaluation Gate 的质量和保护切片门禁使用这个区间的下界：下界低于预定阈值就拦截。工件还会记录
`probability_of_improvement`，即 bootstrap 重采样差值大于零的比例；它帮助诊断当前重采样分布，不能解释成
“新版更好的概率”，也不参与门禁判定。
...
无论使用哪种权重，保护切片仍要单独有足够的 cluster 和 case。把所有用户合并后得到一个窄的总体区间，
不能回答少数高风险用户是否退化；当切片不足预定最低覆盖时，正确结论是补样或阻断，而不是用总体结果填补空白。

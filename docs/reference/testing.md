...
测试通过只说明：给定输入走过当前实现后，结果符合预先写下的手算值、标准定义或独立参考实现。
它不会自动证明理论、事实来源、模型质量、生产性能或系统绝对安全。
...
2. 预期结果来自手推公式、标准、独立实现、人工标注答案，还是被测代码自身？
3. 测试实际证明了什么？
4. 哪些假设和外推没有被验证？
5. 最小正例、边界例与反例分别是什么？

如果 expected value 是从被测实现复制或运行后抄回，测试只能锁定当前行为，不能证明这个行为本身正确。
...
| `contract` | 输入、输出、artifact 或协议边界是否明确，并会在非法或不完整输入出现时停止？ |
...
- 新增关键教学结论，应增加来源清楚的判定依据，例如手算结果、标准定义或独立参考实现。
...
覆盖率不能替代这套判断。未执行代码可能重要，执行过的代码也可能依据了错误的预期结果。

## 首批 claim 审查台账

审查日期：2026-08-18。这里记录每项预期结果的来源和边界，不记录容易过时的“全绿”宣传数字。

| 结论 ID | 教材结论 | 判定依据与测试 | 当前判断 | 不能推出 |
|---|---|---|---|---|
| `ATTN-001` | scaled dot-product attention 对 score 做缩放、softmax，再加权 V | `test_attention_numpy.py` 使用一个可手算的两 key 样例：score 为 `[0, ln(3)]`，概率为 `[1/4, 3/4]`，输出为 `5` | 手算 expected value，不调用第二份 attention 实现 | CUDA kernel、FlashAttention、性能或大模型质量 |
...
| `GEN-001` | 仓库采样策略按 repetition→temperature→top-k→top-p→inverse CDF 执行 | `test_sampling.py` 用精确概率、tie、阈值 crossing、平移不变性和 CDF 边界验证 | 预期结果来自仓库明确声明的采样规则 | Transformers/vLLM/provider 默认顺序或生成质量 |
| `EVAL-001` | Brier、equal-width ECE 与 tie-aware risk–coverage 使用声明的分母 | `test_calibration_metrics.py` 使用可手算的四样本和并列分数样例 | 解析 expected value 与实现分离 | calibration construct validity、总体误差或业务阈值 |
| `EVAL-002` | paired percentile bootstrap 对 paired differences 重采样 | `test_evaluation_statistics.py` 新增 constant-difference 退化分布与 zero-difference strict-improvement 语义；另有使用固定随机种子的非退化样例 | 精确边界补足了原先主要依赖“同 seed 可重复”的弱点 | percentile coverage、case 独立性、cluster 选择或贝叶斯后验概率 |
...
| `TRAIN-002` | token-mean accumulation 必须按整个 update window 的有效 token count 加权 | `test_gradient_accumulation.py` 用 `Fraction` 手算 `[1,3]` token 反例，并与独立 PyTorch backward 实验分层 | 精确手算、错误基线与 framework 实验相互补充 | 随机层、optimizer、DDP/FSDP、AMP、目标模型或质量 |
| `ALIGN-001` | Bradley–Terry/DPO、GAE/PPO 与 categorical policy gradient 使用正文给定符号和 mask | log-two/大 margin 精确值、两步 GAE 手算、正负 advantage clip、full-KL 反例、binary policy-gradient 手算与 finite difference | 核心目标的符号、边界和反例可解释；输入仍是本仓库准备的有限样例 | 完整 RLHF/GRPO、reward 正确、训练稳定或目标模型改善 |
| `DATA-001` | trainer 只能消费已审计 combined artifact 中按原顺序绑定的 train subset | SFT/preference tests 分开篡改 content、order、membership、split、binary label 与 readiness binding | 训练输入和 held-out 隔离契约明确，遇到不一致时会停止 | 数据来源真实、许可充分、标签正确或没有语义泄漏 |
| `DATA-002` | exact/group/near-duplicate 是不同泄漏信号，不能互相冒充 | 固定样例分别制造 exact content 和 group 跨 split；role-tagged user view 的 character 3-gram 可手算为 `14/16`；preference 比较 prompt 与四种 candidate cross-surface | numerator、denominator、normalization profile、serialization view 和 scope 都进入证据；公共 role framing 会影响短文本 | 阈值适合真实域、semantic equivalence、MinHash/ANN 召回或零泄漏 |
...
| `AGENT-004` | local transactional outbox 是 at-least-once，不是远端 exactly-once | SQLite 原子回滚、并发 lease、stale ack 与 success-before-ack crash；provider spy 得到 2 requests、同 key、1 simulated effect | 状态、timeline、调用数与 effect 数分别断言，没有只根据 delivered receipt 判断结果 | 真实 provider honor idempotency key、broker/网络/断电、多区域恢复或外部 effect 唯一 |
...
| `ARTIFACT-003` | MiMo 归档按公开目录记录覆盖，保留缺失与部分采集，并可离线检查文件 | `test_mimo_rl_archive.py` 使用自拟两步与百标签输入、已录制真实目录、空值/漂移/篡改反例及预算检查 | 本地采集、投影、文件与查看器数据一致性契约；真实快照可复算 | 发布者认证、MiMo 训练真实执行、评测质量或远端定时任务已启用 |
| `PROTOCOL-001` | 本仓库提供的 parser、official SDK memory、SDK stdio/HTTP 与 A2A loopback 是不同证据层，不能互借结论 | lifecycle/schema/handler spies、真实 subprocess/pipe/loopback counters、字段要求明确的 receipt 与独立 scope verifier | 相同 JSON 反例保留在独立 loader 上是边界回归，不因为 payload 相同就合并成只测一个实现 | 完整 conformance、远程跨厂商互操作、OAuth/TLS、业务授权、真实 provider 或生产 supervisor |

本轮 1797 个 collected tests 已全部声明证据性质；其中 17 个真实 transport/子进程 case 属于 `extended`，

...
有三类句子不能混写。工程记录说明“固定样例在指定版本下发生了什么”；组织决定说明“谁接受了哪些剩余风险、允许多大范围”；
法律、合同或行业要求记录说明“团队为何认为某项规则在这个地区、角色和事实上适用”，并链接当时的官方来源或正式意见。
模板可以保存第三类判断及其负责人，却不能替代法律意见，也不能把工程测试写成合规证明。
...
继续使用“解析器漏掉非典型经历中的技能”这一风险。用途记录写明系统只能定位证据、拒绝由具名招聘人员作出；影响评估把
漏字段连到错误建议和候选人失去机会；控制证据记录解析器版本、失败简历类别、抽样方法和缺失的群体覆盖；发布决定据此选择
`approve-with-constraints`，只准影子运行，并指定运行负责人能停用路径。这里的“批准”是组织在当前证据下的决定，不是
模型、模板或测试给出的法律结论。

若影子运行再次触发同一漏选，事件记录先固定受影响版本和样本，运行负责人限制流量，工程负责人提交修复与回归产物，
独立审阅人再判断是否恢复。这样每一条字段都对应一个人、一项证据或下一步动作，而不只是填表。

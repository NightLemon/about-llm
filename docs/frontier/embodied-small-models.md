...
把“确认付款”走一遍会更具体。截图显示向商户 A 支付 100 元，本地模型据此提出 `submit(order-17, A, 100)`；
策略把 proposal 与 origin、订单、金额、收款方和短期审批绑定。提交前页面刷新为商户 B，执行器发现当前快照
不再匹配，便停止提交并重新观察，而不是把旧批准套到新页面。即使一次提交得到 HTTP 200，任务也仍处于
`effect unknown`：只有后端按同一幂等键查到订单 `order-17` 对 A 的一笔 receipt，verifier 才能把它写成完成。
这条链中，模型提议、浏览器发出请求、后端确认业务效果是三个可分别失败的事件。

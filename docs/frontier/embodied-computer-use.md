...
部署位置先由数据驻留、授权、离线要求和设备能力过滤。只有仍可行的候选，才比较端到端任务，
而不是只比较一次模型 forward：

| 项目 | 端侧 | 附近工作站 / 边缘 | 云端 |
|---|---|---|---|
| 输入与状态 | 原始数据可留本地 | 需声明局域传输与缓存 | 需满足上传、区域和保留政策 |
| 完整时延 | 准备、模型、工具和本地设备 | 传输、排队、模型与返回 | WAN、排队、模型、工具与返回 |
| 失败恢复 | 断网时可否继续或保存 checkpoint | 节点不可用时怎样迁移 | 请求未知、重试和外部 effect 怎样对账 |
| 成本与能耗 | 设备占用、电量和热限制 | 工作站容量与网络 | 全部尝试费用、配额与 egress |
| 质量分母 | 独立 verifier 的完整 case | 同一任务集与 verifier | 同一任务集与 verifier |

图像压缩或状态裁剪可能缩短单轮上传，却也可能让模型多走几轮。比较时记录实际轮数、纠错、超时、
effect unknown 和按时验证完成率。任何设备带宽、功率或模型速度数字都要绑定具体形态、版本和测量日期。

...
设想一个笔记本助手收到请求：“总结本地合同，并告诉我是否应该付款。”本地处理可以减少数据外发和网络等待，
但所选小模型未必擅长合同判断。换用更适合这个任务的云模型，也会带来网络延迟、费用和数据外发风险。
...
测量能耗时先声明设备从冷启动、空闲还是已热稳态开始，并同时记录墙钟时间、空闲基线、温度/降频状态和电源采样方式。
把被拒绝、超时、本地失败后升级和云端失败的请求保留在端到端延迟与能量分母中；一次成功的短本地生成只能描述该固定
状态和工作负载，不能推出续航、持续功耗或整机能耗。

## 2. 一次本地请求依赖哪些组件

下图只展开本地执行链；失败出口通往[完整的受限路由流程](#local-cloud-release)，后续模型或工具的返回仍需任务验证。
...
flowchart TD
    ...
    K --> V["Verifier"]
    V -->|"pass"| A["Answer"]
    V -->|"fail / uncertain"| F["停止本地路径<br/>转入受限路由或交接"]
```
...
每个终态记录允许外发与实际外发的字段：原始用户数据、已授权的检索证据和本地模型猜测必须分开。若 policy 拒绝外发，
把该请求记为受控拒绝或人工路径，而不是从“升级成功率”分母中消失。这样才能审计 router 是否把不该上传的内容送进
fallback。
...
## 10. 一套可发布的本地—云端架构 { #local-cloud-release }
...
云端模型或工具的返回也要经过任务验证，不能因它来自云端就直接发布。若本地和云端结果都未通过验证，
路由器必须受剩余预算、截止时间与尝试次数限制；用尽后进入人工接管或拒答，并保存已经完成的步骤与未知结果。
若未知的是工具副作用是否已发生，应先查询并对账，不能借“重新路由”再次发送同一动作；读数据或生成候选的升级与此不同。
...
- 能耗测量的初始状态、空闲基线、热状态和采样方法是什么，升级/拒绝/失败请求是否仍在端到端分母中？

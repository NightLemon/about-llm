...
| 常用名称 | 中文解释 |
|---|---|
| 理想选择上限（Oracle@N） | \(N\) 个候选中至少存在一个正确答案，是理想选择器可达到的上限 |
...
比较不同候选数时，固定同一题集、rendered prompt、最大输出、截止时间和可用工具；再明确候选是共享一次 prefill
还是独立调用，并记录并发、缓存与计费口径。每题预算要包含落选候选、verifier、工具、重试和超时所消耗的资源，
否则“同样的 N”并不是公平的 test-time compute 对照。

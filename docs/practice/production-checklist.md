...
把结论写成一次可执行的决定。例如跨租户负例尚未覆盖新版检索器时，值班经理不能把 `go` 留在口头讨论中：发布负责人应记录
`no-go`、受影响版本和补证据负责人；流量负责人保持该版本关闭；工程负责人补充失败与正常样例；独立审阅人确认结果后，
再提交新的决定。若其他门禁已通过但风险能被隔离，可写 `limited-go`，明确 tenant allowlist、5% 流量、观察窗口、自动
停止条件和能执行停止的人。这里的结论是组织的发布判断；测试结果是它的输入，不能单独证明合规或生产安全。

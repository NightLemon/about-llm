...
MCP/A2A 当前只有教材、官方来源台账和实验 fixture 设计路线，不列为工程项目，也不继承 `safe-agent` 的 L2 等级。只有加入固定协议版本的 client/server、认证与 capability negotiation、正负 conformance fixture、真实 transport smoke、取消/乱序/重放故障测试和跨信任域审计后，才可单独评定实现等级。

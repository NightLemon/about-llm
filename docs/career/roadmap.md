...
- **首次阅读**：岗位责任 → 能力等级 → 背景迁移 → JD 解析 → 作品集与面试。
- **完成信号**：能为一个真实 JD 写出责任画像、证据矩阵、主项目选择和仍缺失的下一项实验。
- **卡住时**：先看[学习路径](../guide/learning-paths.md)和[项目索引](../practice/project-index.md)，不要先做时间表。

</div>

本页不是周计划，也不承诺某种证书、项目数量或学习时长能换来 offer。它用于选择主线、读取岗位责任并组织可核验证据；公司、团队和职级定义不同，最终以目标 JD、面试环节和实际交付物为准。
...
岗位名称不是统一标准。“LLM Engineer”可能是 API 后端、检索算法、训练、推理平台或解决方案交付；“算法工程师”也可能主要维护数据和评测。判断岗位时问：
...
你应该选择愿意长期反复解决的失败类型，而不是选择听起来最“前沿”的 title。

## 七类岗位责任画像

下面是稳定的责任切分，不是招聘市场占比或统一职级标准。同一岗位可能跨两到三列。
...
不要用“是否调用 LangChain/Transformers”区分岗位；同一个库可以出现在任何路线，关键是你负责的 invariant 和 decision。
...
“读过论文”通常是 K；“demo 跑通”最多说明部分 I；没有 baseline、held-out 和失败样本就不能声称 V；写了生产设计文档也不等于真实承担过 O。

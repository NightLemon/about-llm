...
- **先修**：已完成至少一个可运行实验，愿意用原始输出和运行记录证明能力。
...
**求职导航**：[深度面试题](interview-questions.md) · [应用与治理题](applied-questions.md) · [编码轮](coding-round.md) · [系统设计题](system-design.md) · [行为面试](behavioral.md)
· [简历项目与证据包](resume-projects.md) · [完整学习路径](../guide/learning-paths.md)
{ .doc-nav }
...
1. 团队最终交付的是研究结果、线上服务，还是某项业务或质量指标？
2. 上游输入由谁拥有，输出交给谁？
3. 哪类失败由这个岗位首先定位和修复？
...
## 把开头这份 JD 真正拆一次 { #worked-example }

假设你已经做了三年后端开发：写过 API、数据库和队列，也处理过接口超时；你会调用模型 API，
但没有系统做过检索评测、GPU 性能分析或模型训练。现在逐句翻译开头那份 JD：

| JD 原文 | 它可能对应的日常工作 | 你现在能拿出的证据 | 还要向团队确认什么 |
|---|---|---|---|
| 负责企业知识助手的研发与优化 | 接入文档、检索、生成、权限和业务 API | 后端服务、数据流、故障处理 | 主要优化回答质量，还是负责整套线上服务？ |
| 熟悉 RAG、Agent、vLLM、LangChain，了解 PEFT | 这些是候选工具，不一定都是主要职责 | 模型 API 和后端框架经验 | 哪个组件由本岗位维护？需要训练或管理 GPU 吗？ |
| 保障系统稳定性和回答质量 | 既要处理 `504`，也要解释错误答案来自哪一层 | 可观测、回滚和接口测试 | 是否承担 on-call？回答质量用什么 case 和指标判断？ |

这位候选人不应同时转向训练、推理内核和 Agent 研究。更合理的决定是：

- **主路线选 LLM 应用与 RAG**：已有后端能力可以直接支撑 API、权限、状态和故障处理；
- **相邻路线选 Agent 平台**：队列、幂等和恢复经验可以迁移到工具调用；
- **暂不声称模型训练或推理优化能力**：目前没有数据实验、checkpoint 或 GPU 测量证据。

接下来只补两个能够推翻自己判断的实验：

1. 在[可诊断 RAG](../practice/projects/rag-foundations.md)中固定一组问题，分别观察检索、重排、引用和权限过滤。
   如果无法解释一条错误答案来自哪一层，说明 RAG 还没有学到能面试的深度。
2. 在 [Safe Agent](../practice/projects/safe-agent.md)中运行一次退款任务，解释审批、幂等和超时对账。
   如果只能讲 Prompt，无法说明重复退款怎样被阻止，就不应把 Agent 平台列为强项。

完成后，作品集可以用 RAG 作主项目、Safe Agent 作相邻项目，再用
[Transformers Basics](../practice/projects/transformers-basics.md)证明自己理解模型接口下面的分词、注意力和生成协议。
这已经形成一条清楚的后端转型路线，不需要再做四个换壳聊天机器人。

后面的岗位表和能力矩阵用于检查这个决定。第一次阅读时只看与“应用 / RAG / Agent”有关的列；
准备其他岗位时，再换到相应列。
...
LangChain、Transformers 或 vLLM 都可能出现在多条路线中。真正区分岗位的是：
哪些性质必须始终成立，以及失败发生时需要你作出什么决定。
...
| 级别 | 你实际做了什么 |
|---|---|
| K | 能解释 BM25 和重排器分别解决什么问题，也知道反例 |
| I | 从文档摄取到带引用回答，跑通一条最小链路 |
| V | 固定相同 case，对比基线和新方案；通过消融与留出数据说明改动来自哪里 |
| O | 为服务定义 SLO 和权限边界，并实际处理发布、回滚和事故 |

这四级描述的是证据覆盖到哪里，不会随工作年限或职称自动升级。

### 角色能力矩阵

下表只是准备面试的起点，应按目标 JD 调整。`V/O` 表示通常需要拿出验证记录，
或说明自己怎样持续负责这项能力；它不是所有公司的统一门槛。
...
优先考虑 LLM 应用、Agent 平台或评测平台。API、数据库、队列、可观测和运维经验都可以直接复用。
你需要补的是模型特有边界：分词器与对话模板怎样改变输入，非确定生成怎样测试，以及 RAG 如何分层评测。
作品集还应展示权限和发布门禁，让成熟后端能力真正支撑模型功能。

### 搜索、推荐或数据工程师

优先考虑 RAG 搜索、评测数据或训练数据。检索排序、特征管道、A/B 实验和指标设计都是现成优势。
下一步要理解检索结果怎样装入模型上下文，生成回答是否忠于证据，以及模型评审器在哪些 case 上不可靠。
多租户权限与 token 成本则把离线算法连接到真实服务。

### 传统 ML/深度学习工程师

优先考虑模型训练、RAG 算法或评测。训练、优化和实验设计可以继续复用，但公开基准只是起点。
还要补上对话模板如何形成训练输入、工具和 RAG 系统怎样改变模型行为，以及目标服务有哪些延迟和容量约束。
数据来源、模型版本和真实工作负载也要进入实验记录。

### 系统、HPC、编译器或 SRE 工程师

优先考虑推理系统、训练平台或 Agent 平台。性能分析、并行、内存、调度和可靠性是直接优势。
需要补足 Transformer 与 KV cache 的状态变化，并理解生成质量和量化回归怎样约束性能优化。
任何吞吐提升都要绑定模型版本，并通过质量门禁后再比较。

### 安全、隐私或治理工程师

优先考虑 AI 安全平台、Agent 或 RAG 控制面，以及评测治理。威胁建模、身份权限、审计和事故响应能力可以直接复用。
模型带来的新问题包括输出不确定、提示注入、候选工具与真实动作混淆，以及训练或检索数据污染。

先把每种风险写成可以运行的测试样例。模型是否拒答只是观察信号，真正的边界仍由权限系统和控制面实现。
...
例如，JD 写着“优化大模型推理性能”，简历只写“使用 vLLM”还回答不了你优化了什么。
先把实验拆成四项：

1. 固定模型 checkpoint、推理框架版本和请求负载；
2. 保存首 token 延迟、后续 token 延迟、吞吐、峰值显存和质量变化；
3. 判断瓶颈来自 prefill、decode、请求排队还是 KV cache；
4. 说明自己做到了哪一层。若没有分析 CUDA kernel，就把结论限定在框架和请求层。

JD 中的技术栈是约束，不是完整能力定义。使用过某个框架，但说不清输入输出、失败模式和替代方案，证据仍然很弱。
反过来，如果你用另一个框架解决过同层问题，并保存了基准结果和迁移分析，这可以作为相邻证据向面试官解释。
...
2. 是否有一项可以深挖到原始输出和运行记录的主证据；
3. 缺失技术是同层工具迁移，还是缺少整个能力域；
4. 对地域、许可、学历、工作授权或 on-call 等硬条件是否满足；
5. 能否诚实解释未做过的规模和目标环境。

一种工具 API 通常可以在短期内补齐。实验设计和端到端负责经验需要更长时间积累，但具体要求仍以目标团队为准。
...
到 L4 时，项目已经讨论容量、安全、监控、回滚和成本。真实生产流量、组织流程和可用性，
仍要由实际运行记录证明。
...
简历中的精确数字应能追溯到原始输出、代码版本、运行配置和失败样本。缺少其中任何一项时，
先补齐记录，或者删掉无法复核的数字。截图可以辅助展示，但不能替代可重新计算的结果。
...
## 选择团队时也要做反向评估 { #reverse-evaluation }
...
研究岗位可以用可复现实验和被采用的方法证明价值，平台岗位可以用内部用户、迁移结果和故障恢复记录证明价值。
无论是否直接承载生产流量，都要问团队怎样定义成功、是否保留原始结果，以及失败能否被及时发现。

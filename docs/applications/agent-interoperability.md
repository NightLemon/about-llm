...
flowchart TD
  ...
  A --> V["6. 本地查询并核对取件产物（artifact）<br/>通过后才向用户确认"]
```

图中有两个容易被误判为“业务已经成功”的时刻：MCP 返回可用时段，A2A task 进入 `completed`。前者只是一条
观测（observation），后者只带回一份待验收的协议产物（`artifact`）。售后 Agent 必须完成本地核对，才能向用户确认预约。

## 先区分三层契约

| 层次 | 常见对象 | 它主要解决什么 | 仍由业务系统负责什么 |
|---|---|---|---|
| 模型服务 API | 消息、内容块、流事件 | 怎样调用模型或托管 Agent API | 跨服务提供方语义、租户与业务权限 |
...
适配器（adapter）把外部协议转换成这些规范化类型。外部 JSON 不能直接变成授权对象，
...
建立连接时，主机还要记录服务端标识、协议版本和双方协商出的能力。工具定义本身也要有结构版本（schema revision），
...
模型可以提出查询 `order-1001` 的可用时段。本地运行时（runtime）随后要回答：

1. 订单由哪个 tenant 和 subject 拥有？
2. 当前 capability 是否允许读取这笔订单的取件信息？
3. 当前服务端标识与结构版本是否在允许范围内？
4. 缓存或重放会不会绕过已经撤销的权限？
...
一次委派由消息（`message`）发起，随后通过任务（`task`）、状态更新和产物（`artifact`）跟踪结果。这种结构可以表达长任务和流式更新。
...
规范化后的产物，再使用先前保存的订单、时段、批准和业务副作用 ID 完成对账。
...
遇到未知字段、状态或能力时，适配器应停止处理，或把原始协议产物（`artifact`）放入受控审计存储。业务逻辑只读取
...
| A2A 委派取件 | 发送 7 和 5，取得 `completed` 任务与 `sum=12` 产物 | Agent Card、`SendMessage`、`GetTask` 与本地产物验证可执行 |
...
| 加法产物的本地验证器 | 多 worker、跨区域恢复与真实业务副作用 |
...
2. A2A 任务显示 `completed` 时，为什么仍要查询或验证取件产物？

...
一次离线恢复演练应有可判定的结束条件：在隔离位置恢复后，先核对 manifest、schema、来源与 chunk 数量，
再用固定的授权请求确认结果 ID、引用来源和拒答分支。任一步失败时，不要切换 read alias；保留失败工件，
回到上一已发布版本，并从第一个不匹配的边界开始修复。这样“备份文件能打开”和“可以安全恢复服务”不会被混成同一个结论。

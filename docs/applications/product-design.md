...
“取消尚未开始的动作”只适用于仍在本地等待执行的授权：产品可以撤销这次授权或停止排队。它不能撤回已经发送给
支付服务的请求。进入 Unknown 后，按钮也不能把原 `call_id` 再提交一次；系统必须先用该业务编号查询外部状态，
再显示已受理、确认未受理后的新操作，或人工接管。
...
<div style="min-width: 500px;" markdown="1">
...
因此，界面应把三件事分开呈现：模型整理出的退款草稿、用户对该草稿的授权、支付服务确认的 receipt。前两件事
让系统有资格尝试，第三件事才说明外部效果已经发生。把这三个标签留在同一张详情页上，客服才能解释为何超时后
仍不能承诺“退款成功”。
...
本页交付的是用户可见状态、证据展示、确认、取消、纠错与人工接管设计。验收时仍要同时覆盖任务、证据、动作、
可访问性、系统与安全，但各指标的定义和统计不在这里再展开：

| 要验证什么 | 权威入口 |
|---|---|
| 构念、标注、分母与 A/B 设计 | [评测测量学](../quality/evaluation-measurement.md)与[评测方法](../quality/evaluation-methodology.md) |
| 隐私、遥测、保留与群体影响 | [隐私与公平](../quality/privacy-fairness.md) |
| RAG 引用和拒答 | [RAG 生成](rag-generation.md) |
| Agent proposal、approval、effect 与恢复 | [Agent Runtime](agent-runtime.md) |
| 服务状态、取消与发布 | [服务与可观测性](../systems/serving.md) |

先用 recorded outputs 做原型和失败场景可用性测试，再进入离线回放与小流量发布。满意度不能代替正确性；任何总体
改善也不能抵消越权、重复副作用或关键群体失败。仓库当前没有真实 UI、辅助技术审计或线上用户实验，因此这些结论
仍需在目标产品中验证。
...
6. 为这套界面写一份评测交接说明，指出哪些问题应由测量学、隐私和服务专题回答。

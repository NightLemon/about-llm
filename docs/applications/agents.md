...
最危险的恢复情形是：本地等待退款服务响应超时，而服务端是否已受理仍未知。此时本地状态是 `unknown`，既不能写成失败，也不能直接再发一次。控制面先用稳定幂等键查询远端状态；只有服务契约明确允许时才重试。详见[Exactly-once 幻觉与 outbox](agent-runtime.md#exactly-once)。
...
MCP 主要连接 AI 应用与工具、资源和提示模板；A2A 主要描述独立 Agent 的发现、任务和协议产物（`artifact`）。它们统一通信方式，却不会自动建立业务信任。

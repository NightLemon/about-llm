...
两个工具都能并行调用，不表示它们消耗的 CPU、环境内存或费用消失。设模型前后两段各耗时 2 秒和 3 秒，
工具 A、B 分别耗时 6 秒和 10 秒。工具有依赖时关键路径为 \(2+6+10+3=21\) 秒；
它们独立且资源允许并行时，关键路径为 \(2+\max(6,10)+3=15\) 秒，但工具工作量仍是 16 tool-seconds。

环境若从任务开始到提交一直保留 2 GiB 内存，15 秒并行路径仍占用 30 GiB·s。
报告完整任务时应把以下项目分列：

| 账本 | 聚合方式 | 用来回答什么 |
|---|---|---|
| 关键路径 wall time | 依赖 DAG 上最长路径 | 用户要等多久 |
| CPU/GPU/tool work | 所有实际执行段求和 | 资源与费用消耗多少 |
| 环境内存时间 | \(\int M(t)\,dt\) | 有多少容量被驻留状态占用 |
| 外部等待 | 单列 provider、网络和审批 span | 等待是否仍持有环境或配额 |

并行策略必须同时满足依赖和资源约束。两个工具若争用同一独占环境，公式中的 `max` 应恢复为串行和。

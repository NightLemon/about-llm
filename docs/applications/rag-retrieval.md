...
多租户/ACL 场景中的“过滤后打分”还包括 corpus statistics。若先在全库计算 IDF 与平均长度，再只跳过越权文档，隐藏文档仍会改变可见 score。当前 `BM25Index/authorized-statistics-v2` 先得到授权集合，再仅用该集合计算 IDF、平均长度和排序分数；回归测试要求加入跨租户或 ACL-blocked 文档后，可见结果的 ID 与 score 都逐项不变。

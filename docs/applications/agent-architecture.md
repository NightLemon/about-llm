...
复杂分支图保留完整状态名；窄屏可在图内横向滚动，配合上面的六步循环阅读。
...
下面是一份**设计示意**，不是本仓库的 handoff 实现，也不是 A2A 消息格式。Agent A 把待核对的退款交给 Agent B 时，
控制面至少保存：

```text
handoff_id / task_id / task_version / current_owner = B
objective = 核对 order-1001 的退款申请是否已受理
pending_call = call_id + execution_fingerprint + provider idempotency key
budget_and_deadline = 剩余预算、查询截止时间
evidence = 本地 timeout、ledger pending、尚无匹配 receipt
acceptance = B 已读取当前版本，并承诺先查询、再由 verifier 判定
```
...
任务状态回答“这次退款进行到哪里”，必须支持恢复与对账；长期记忆帮助未来任务使用过去信息，写入条件、来源、
有效期、权限、纠正和删除都更严格。工作/情景/语义/程序性记忆的 schema、冲突与隔离统一见
[对话状态与记忆](conversation-state-memory.md)。本页只保留二者不能互相替代这一架构边界。
...
对用户应报告当前未知状态、可恢复的任务 ID 和下一次查询或人工接管的条件。
...
工作进程已把请求发送给支付服务、但本地确认丢失时，结果仍未知，发件箱可能再次投递同一请求。为避免重复退款，
重投必须继续使用稳定的业务效果 ID，并让支付服务把它作为幂等键。系统最后还要查询回执，由验证器对账。
...
你应能用同一张控制循环图解释：何时有限状态机已经足够，何时才需要模型规划；任务状态为何不能只放在对话中；
停止、完成提议与完成验证为什么是三个不同判断。面试题目与练习统一见[面试题库](../career/interview-questions.md)。

...
- **首次阅读**：先读第 1–4、6、8–9 节，跟随代码题看采样、评分与更新；Actor–critic、GAE 和 PPO 留到第二遍。
...
想看看这个循环怎样反映在真实训练曲线上，可读[MiMo RL 截图讲解](../models/mimo-rl.md)：
对着一张原站成功率图，算懂 0.615，并解释曲线的上升与回落。
完整的[十一篇看图专题](../models/mimo-rl-series.md)还会讲奖励与优势、参数更新、采样流水线、训练成本和检查点评测。
...
!!! note "第二遍再读"
    第一次只需知道 value/baseline 用来降低方差，并能区分真正终止与长度截断。需要实现 PPO 时再推导 TD、GAE 和 mask。
...
## 6. 保存回答文本，还不等于保存了训练数据身份 {#rollout-identity}
...
| 指标 | 它实际回答什么 |
|---|---|
| 理想选择上限（Oracle@N） | \(N\) 个候选里是否至少存在一个真正正确的答案，是理想选择器的上限 |
...
使用第 10 节的反馈类型表做选择：基础行为未稳定先做 SFT；固定偏好对从 DPO 类方法起步；
只有当前策略的实际回答需要进入环境或 verifier 时，才引入 online RL。若瓶颈来自权限、实时事实、
副作用可靠性、错误测试或答案泄漏，应先修系统与评测，增加 RL 复杂度不会修复错误目标。

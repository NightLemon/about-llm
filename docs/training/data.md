...
- **适合读者**：数据工程、治理和训练流水线负责人。
- **先修**：数据集、JSONL、hash、train/test 划分的基本概念。
- **首次阅读**：数据契约 → lineage → 去重 → 污染 → 发布不变量。
- **完成信号**：能产出可追溯 manifest，并证明删除、切分和版本边界。
- **卡住时**：先读[机器学习与深度学习](../foundations/ml-dl.md)的数据划分部分。

...
- **先修**：知道 JSONL、哈希、训练集/验证集/测试集划分和 tokenizer 的基本作用。
...
评审模型可能偏好较长、较标准或与自身风格相似的文本，这种偏好会逐渐写进训练分布。
...
表中的 label 放在**目标 token 所在的位置**：位置 1 的 `12` 由位置 0 的 A1 预测，
因此位置 3 的 `-100` 排除了位置 2 的 `EOS(A) → B1` 这项损失。这种数组表示法与许多
训练实现内部把 logits 和 labels 错开一位的做法配合使用；核对时要同时打印两者，不能只看 label 数组。

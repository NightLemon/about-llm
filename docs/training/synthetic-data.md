...
| 候选 | 实际发生了什么 | 这一步怎样处理 |
|---|---|---|
| `syn-001` | 两项检查通过，记录中带有人审标记 | 进入 verifier-eligible 集合 |
...
所以这批数据有四个候选、两个通过 gate 的记录，却只有一份 byte-exact（逐字节）不同内容。
通过率 `2 / 4 = 50%` 比较的是记录数；去重后可用的不同内容数与原始候选数之比是 `1 / 4 = 25%`，
与通过记录数之比则是 `1 / 2 = 50%`。后两个比例描述内容去重后的保留量，不表示每条记录有多少概率“独一无二”。
`syn-002` 还有一项相关性风险：生成器和 grounding verifier 都写着 `teacher@v1`。

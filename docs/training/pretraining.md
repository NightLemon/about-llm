...
```mermaid
flowchart TD
    A["登记来源与原始快照"] --> B["解析、过滤、去重与 split"]
    B --> C["分词、拼接与 mask"]
    C --> D["有效 token loss<br/>梯度汇总"]
    D --> E["提交参数更新<br/>写入 checkpoint"]
```

沿这条链，一条原始记录经过处理后变成 token，mask 再决定哪些目标参与 loss。本章用这些有效目标的计数表示监督进度；
读取量、padding 和实际计算的位置数还要另记。Checkpoint 把已提交的参数更新与数据位置绑定起来。
缺少其中的身份或计数，`NaN` 和恢复分叉就可能变成无法重现的偶发故障。
...
[Scaling law（规模化规律）](../core/scaling.md)研究发现：在固定模型族、数据和训练范围内，loss 往往会随参数量、数据量和计算量呈现

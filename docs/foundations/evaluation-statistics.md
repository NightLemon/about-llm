...
- **首次阅读**：Estimand → 配对差值 → 区间 → 切片与多重比较；发布流程放到评测方法页。
...
本例中它就是 `9,995 / 10,000`；均值恰为 0 的重采样不计入分子，因为程序使用严格的 `> 0`。
...
大模型评分（LLM-as-a-Judge）只是产生观测值的一种方法。裁判版本、rubric、顺序偏差、人工参照与效度检查由
[评测测量学](../quality/evaluation-measurement.md)负责；本页从已经定义好的逐 case 结果开始，处理配对、区间、
切片与重复查看。无法解析、平局和评分失败仍须留在统计分母中。
...
本页交付的是 effect、区间、分母、关键切片和统计假设。怎样把它们与安全、延迟、成本及人工复核组成发布门禁，
由[评测方法](../quality/evaluation-methodology.md)统一定义；可运行的产物链见
[Evaluation Gate](../practice/projects/evaluation-gate.md)。门槛仍须在查看候选结果前固定。

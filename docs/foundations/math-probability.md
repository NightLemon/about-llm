...
这里要求凡是 \(p(x)>0\) 的位置都有 \(q(x)>0\)；否则对应项的代价是无穷大。\(p(x)=0\) 的项按
\(0\log q(x)=0\) 处理。若 \(q\) 给真实常见结果的概率太低，平均编码代价会增大。一个训练 token 的
one-hot label 给出一个观测到的 NLL；把许多 token 的 NLL 按既定分母平均，才是训练中使用的经验交叉熵。

...
LLM 公式常把许多简单步骤压缩在一行里，因此初看时信息量很大。本章把这些步骤展开，只追踪一次预测：
...
向量 \([1,1]\) 有两个元素，shape 是 <code>[2]</code>。为了做矩阵乘法，下面把它临时写成只有一行的
矩阵，shape 才是 <code>[1,2]</code>。下面的权重矩阵有 2 行、3 列，shape 是 <code>[2,3]</code>：

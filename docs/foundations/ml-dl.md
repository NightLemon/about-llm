...
对语言模型，一条人类文本不会直接成为 loss。它先由 tokenizer 固定为一列 IDs；训练代码再从同一列 IDs 构造
“当前位置输入 → 下一个目标”的错位配对，并用 loss mask 定义分母。服务生成时则把尚未结束的模板化 ID 前缀送入模型，
取最后一个位置的 logits 选出一个 ID，再把该 ID 追加回前缀。下一页用 `你好🙂!` 把这两种用途逐位置展开：
[NLP 与语言建模](nlp.md#shift-and-mask)负责训练样本，[Tokenization](../core/tokenization.md)解释 ID 从何而来，
[Transformer](../core/transformer.md)解释追加后的序列如何经过 attention。
